"""Command-line tool functionality for merge."""
...
from cellcommander.merge.checkpoint import create_workflow_hashcode
...
def setup_and_logging(args):
    ...
    log_file = os.path.join(output_dir, "merge.log")
    ...
    formatter = logging.Formatter("cellcommander:merge: %(message)s")
    ...
    logger.info("Command:\n" + " ".join(["cellcommander", "merge"] + sys.argv[2:]))
...
def main(args):
    ...
    # Run the tool.
    from cellcommander.merge.run import run_merge
    run_merge(args)
    file_handler.close()
