"""Command-line tool functionality for integrate."""
...
from cellcommander.integrate.checkpoint import create_workflow_hashcode
...
def setup_and_logging(args):
    ...
    log_file = os.path.join(output_dir, "integrate.log")
    ...
    formatter = logging.Formatter("cellcommander:integrate: %(message)s")
    ...
    logger.info("Command:\n" + " ".join(["cellcommander", "integrate"] + sys.argv[2:]))
...
def main(args):
    ...
    # Run the tool.
    from cellcommander.integrate.run import run_integrate
    run_integrate(args)
    file_handler.close()
